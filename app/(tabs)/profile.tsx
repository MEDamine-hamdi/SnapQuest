...
export default function ProfileScreen() {
  ...
  const [user, setUser] = useState<any>(contextUser); // use stored user immediately
  ...
  const level    = getCurrentLevel(user.total_xp ?? 0);
  const progress = getXPProgress(user.total_xp ?? 0, level);

  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.avatarCircle}>
          {/* fixed: was user.name, field is now username */}
      ...
      <View style={styles.xpCard}>
        <View style={styles.xpRow}>
          <Text style={styles.xpLabel}>⚡ {user.total_xp ?? 0} XP</Text>
          <Text style={styles.xpNext}>
            Prochain niveau : {level.maxXP === Infinity ? 'MAX' : `${level.maxXP} XP`}
          </Text>
        </View>
      ...
      <View style={styles.statsRow}>
        {[
          { label: 'Défis',  value: user.challenges_completed ?? 0, icon: '🏆' },
          { label: 'Streak', value: `${user.streak ?? 0}j`,         icon: '🔥' },
...
const styles = StyleSheet.create({
  ...
  centered:       { flex: 1, backgroundColor: '#0a0b0f', justifyContent: 'center', alignItems: 'center' },
